class Solution{
    ...
    int shortestCommonSupersequence(string str1, string str2, int n, int m){
         ...
         vector<vector<int>>Dp(n+1,vector<int>(m+1,-1));  //Using Memoization 
         memo(str1,str2,n,m,Dp);
       int lenOfSuperSequence_Memo = n+m-Dp[n][m] ; //legth of shortestCommonSupersequence :- n+m-Length of LCS(str1,str2)  
      return lenOfSuperSequence_Memo;
      ...
      //Tabulation Approach:- 
        ...
        vector<vector<int>>dp(n+1,vector<int>(m+1,-1)); // for tabulation
