 #include<bits/stdc++.h>  
    ...
    long solve2(int ind,int *coins, int amount,vector<vector<long>>&dp,int n){
           //when amount exhausted 
           if(ind==n-1){
             //if by using that last coin amount can be exhausted 
              //its possible therefore return 1 else not Possible
...
{
        //  long coin = solve(0,coins,amount,n) ;
       // return coin ; //Approach-1 Recursion

       // vector<vector<long>>dp(n,vector<long>(amount+1,-1));
       //long coin2 = solve2(0,coins,amount,dp,n);
       //return coin2 ; //Approach-2 Memoization
   
      //long coin3 = tabulation(coins,amount,n);
      // return coin3 ; //Approach-3 Tabulation

     // long coin4 = tabulationWithSpaceOptimization(coins,amount,n);
     //  return coin4 ; //Approach-4 tabulationWithSpaceOptimization

       long coin5 = tabulationWithSpaceOptimization(coins,amount,n);
